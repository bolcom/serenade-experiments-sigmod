...
public class PredictionsWriter {
    ...
    public PredictionsWriter(String outputFilename) throws FileNotFoundException {
    ...
    public void appendLine(List<Recommendation> predictions, List<Long> nextItems) {
        String reco = String.join(",", predictions.stream().map((Recommendation r) -> String.valueOf(r.getItemId())).collect(Collectors.toList()));
        String nexts = String.join(",", nextItems.stream().map((Long item) -> String.valueOf(item)).collect(Collectors.toList()));
        outputPrintWriter.write(reco + ";" + nexts + "\n");
    }
