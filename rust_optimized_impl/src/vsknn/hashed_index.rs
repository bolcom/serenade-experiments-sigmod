...
use crate::vsknn::index::{VSKnnIndex, idf, SessionTime};
use std::collections::BinaryHeap;
use crate::vsknn::SessionScore;
use crate::io::{ItemId, TrainingSessionId, Time};

pub struct HashedVSKnnIndex {
...
impl VSKnnIndex for HashedVSKnnIndex {
    ...
    fn find_neighbors(
        ...
        for session_item in evolving_session.iter() {
            match self.sessions_for_item(session_item) {
                Some(sessions) => {
...
impl HashedVSKnnIndex {

    pub fn new(interactions: Vec<(TrainingSessionId, ItemId, Time)>, sample_size_m: usize, max_qty_session_items: usize) -> Self {
        ...
        let mut historical_session_index: HashMap<TrainingSessionId, HashSet<ItemId>> = HashMap::new();
        ...
        for (session_id, item_id, order) in interactions.into_iter() {
            ...
            let item_sessions = historical_item_index.entry(item_id).or_insert(HashSet::new());
        ...
        let item_idfs: HashMap<u64, f64> = historical_item_index.iter()
