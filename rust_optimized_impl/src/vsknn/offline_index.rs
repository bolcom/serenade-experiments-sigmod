...
use crate::vsknn::hashed_index_var::read_from_file;
use crate::vsknn::hashed_index_var::prepare_hashmap;
...
pub struct OfflineIndex {
    ...
    session_to_max_time_stamp: Vec<u32>,
    item_to_idf_score: HashMap<u64, f64>,
    session_to_items_sorted: Vec<Vec<u64>>,
    pub(crate) training_data_stats: TrainingDataStats,
    item_to_product_attributes: HashMap<u64, ProductAttributes>,
}

impl OfflineIndex {

    pub fn new_from_csv(path_to_training: &str, n_most_recent_sessions: usize) -> Self {
        let start_time = Instant::now();
        println!("reading training data, determine items per training session {}", &path_to_training);
        let data_train = read_from_file(&path_to_training);
        let (historical_sessions_train,
            _historical_sessions_id_train,
            historical_sessions_max_time_stamp, training_data_stats) = data_train.unwrap();
        ...
        let (item_to_top_sessions_ordered, item_to_idf_score, _session_to_items_sorted, item_to_product_attributes) =
    ...
    pub fn new(base_path: &str) -> Self {
        println!("reading training data, determine items per training session {}", &base_path);
        let start_time = Instant::now();
        let (item_to_top_sessions_ordered, item_to_idf_score, item_to_product_attributes) = create_item_indices_from_avro(&*(base_path.to_owned() + "/itemindex/"));
        println!("indexing item indices: {} secs", start_time.elapsed().as_secs());
        let start_time = Instant::now();
        let (session_to_items_sorted, session_to_max_time_stamp) = create_session_indices_from_avro(&*(base_path.to_owned() + "/sessionindex/"));
        println!("indexing session indices: {} secs", start_time.elapsed().as_secs());
        ...
        fn create_item_indices_from_avro(dir: &str) -> (HashMap<u64, Vec<u32>>, HashMap<u64, f64>, HashMap<u64, ProductAttributes>) {
            ...
            for path in dir_entry {
                ...
                if full_path_to_file.ends_with(".avro") {
                    ...
                    for value in reader {
                        ...
                        match parse_result {
                            Ok(item_index) => {
                                let top_sessions_ordered = item_index.session_indices_time_ordered.iter().map(|x| x.clone() as u32).collect_vec();
                                item_to_top_sessions_ordered.insert(item_index.ItemId as u64, top_sessions_ordered);
                                ...
                                item_to_product_attributes.insert(item_index.ItemId as u64, attributes);
                            }
                            Err(err) => {
                                println!("{:?}", err);
                                break;
            ...
            println!(" item_to_top_sessions_ordered.len():{}", item_to_top_sessions_ordered.len());
            (item_to_top_sessions_ordered, item_to_idf, item_to_product_attributes)
        }
            ...
            for path in dir_entry {
                ...
                if full_path_to_file.ends_with(".avro") {
                    ...
                    for value in reader {
                        ...
                        match parse_result {
                            Ok(session_index) => {
                                let session_items_asc = session_index.item_ids_asc.iter().map(|x| *x as u64).collect_vec();
...
impl VSKnnIndexNew for OfflineIndex {
    fn items_for_session(&self, session: &u32) -> &Vec<u64> {
        &self.session_to_items_sorted[session.clone() as usize]
    }
                        ...
                        Some(similar_sessions) => {
                            ...
                            'session_loop: for session_id in similar_sessions {
                                match session_similarities.get_mut(session_id) {
                                    ...
                                    None => {
                                        let session_time_stamp = self.session_to_max_time_stamp[*session_id as usize];
        ...
        for (session_id, score) in session_similarities.iter() {
            if closest_neighbors.len() < k {
                ...
                if score < &top.score {
                    let scored_session = SessionScore::new(*session_id, *score);
                    *top = scored_session;
                } else if (score == &top.score) && (self.session_to_max_time_stamp[*session_id as usize] > self.session_to_max_time_stamp[top.id as usize]) {
                    let scored_session = SessionScore::new(*session_id, *score);
                    *top = scored_session;
                }
            }
