use crate::vsknn::index_new::{VSKnnIndexNew, SessionTime};
...
impl HashIndexVar {
    ...
    pub fn new(path_train: &str, n_most_recent_sessions: usize) -> Self {
        ...
        println!("reading training data, determine items per training session {}", &path_train);
        let data_train = read_from_file(&path_train);
        let (historical_sessions_train,
            _historical_sessions_id_train,
            historical_sessions_max_time_stamp, _training_data_stats) = data_train.unwrap();
        println!("reading training data, determine items per training session:{} micros", start_time.elapsed().as_micros());

        let start_time = Instant::now();
        println!("prepare indexes");
        let (item_to_top_sessions_ordered, item_to_idf_score, session_to_items_sorted, item_to_product_attributes) =
            prepare_hashmap(&historical_sessions_train, &historical_sessions_max_time_stamp, n_most_recent_sessions);
        println!("prepare indexes:{} micros", start_time.elapsed().as_micros());
...
pub fn read_from_file(path: &str) -> Result<(Vec<Vec<u64>>, Vec<Vec<usize>>, Vec<u32>, TrainingDataStats), Box<dyn Error>> {
    ...
    reader.deserialize()
        .for_each(|result| {
            if result.is_ok() {
                ...
                let (a_session_id, a_item_id, a_time): (usize, usize, usize) =  (raw.0, raw.1, raw.2.round() as usize);

                session_id.push(a_session_id.clone());
                item_id.push(a_item_id.clone());
                time.push(a_time.clone());
    ...
    let qty_unique_session_ids = session_id_unique.len();

    // Get unique item ids
    let mut unique_item_ids = item_id.clone();
    unique_item_ids.sort();
    unique_item_ids.dedup();
    let qty_unique_item_ids = unique_item_ids.len();

    let min_time = time.par_iter().min().unwrap();
    let min_time_date_time =
        NaiveDateTime::from_timestamp(min_time.clone() as i64, 0);
    let max_time = time.par_iter().max().unwrap();
    let max_time_date_time =
        NaiveDateTime::from_timestamp(max_time.clone() as i64, 0);

    // Create historical sessions array (deduplicated), historical sessions id array and array with max timestamps.
    //let mut i: usize = 0;
    let mut historical_sessions: Vec<Vec<u64>> = Vec::with_capacity(session_id_unique.len());
    let mut historical_sessions_id: Vec<Vec<usize>> = Vec::with_capacity(session_id_unique.len());
    let mut historical_sessions_max_time_stamp: Vec<u32> = Vec::with_capacity(session_id_unique.len());
    ...
    let training_data_stats = TrainingDataStats {
        ...
        qty_unique_session_ids: qty_unique_session_ids,
        qty_unique_item_ids: qty_unique_item_ids,
        min_time_date_time: min_time_date_time.clone(),
        max_time_date_time: max_time_date_time.clone(),
        session_duration_p05: session_duration_p05,
        session_duration_p25: session_duration_p25,
        session_duration_p50: session_duration_p50,
        session_duration_p75: session_duration_p75,
        session_duration_p90: session_duration_p90,
        session_duration_p95: session_duration_p95,
        session_duration_p99: session_duration_p99,
        session_duration_p99_5: session_duration_p99_5,
        session_duration_p100: session_duration_p100,
        qty_events_p05: qty_events_p05,
        qty_events_p25: qty_events_p25,
        qty_events_p50: qty_events_p50,
        qty_events_p75: qty_events_p75,
        qty_events_p90: qty_events_p90,
        qty_events_p95: qty_events_p95,
        qty_events_p99: qty_events_p99,
        qty_events_p99_5: qty_events_p99_5,
        qty_events_p100: qty_events_p100,
    };

    Ok((historical_sessions, historical_sessions_id, historical_sessions_max_time_stamp, training_data_stats))
}
...
pub(crate) fn prepare_hashmap(historical_sessions: &[Vec<u64>], timestamps: &[u32], n_most_recent_sessions: usize) ->  (HashMap<u64, Vec<u32>>, HashMap<u64, f64>, HashMap<u32, Vec<u64>>, HashMap<u64, ProductAttributes>) {
    /***
     Returns
     item_to_top_sessions_ordered: HashMap<u64, Vec<u32>>
     item_to_idf_score: HashMap<u64, f64>
    ...
    let historical_sessions_values_sorted: Vec<u64> = historical_sessions_indices.iter().map(|&i| historical_sessions_values[i].clone() as u64).collect();
    let historical_sessions_session_indices_sorted: Vec<u32> = historical_sessions_indices.iter().map(|&i| historical_sessions_session_indices[i].clone() as u32).collect();
    let historical_sessions_timestamps_sorted: Vec<u64> = historical_sessions_indices.iter().map(|&i| historical_sessions_timestamps[i].clone() as u64).collect();
    ...
    for current_item in unique_items.iter() {
        ...
        let current_item_timestamps: Vec<u64> = historical_sessions_timestamps_sorted[left_index..right_index + 1].to_vec();
        let current_item_similar_sessions_ids: Vec<u32> = historical_sessions_session_indices_sorted[left_index..right_index + 1].to_vec();
        ...
        let mut current_item_similar_sessions_id_sorted: Vec<u32> = timestamp_indices.iter().map(|&i| current_item_similar_sessions_ids[i].clone() as u32).collect();
        ...
        let idf_score = (historical_sessions_values_sorted.len() as f64 / current_item_timestamps.len() as f64).ln();
    ...
    (item_to_top_sessions_ordered, item_to_idf_score, session_to_items_sorted, item_to_product_attributes)

}

impl VSKnnIndexNew for HashIndexVar {
    ...
    fn find_neighbors(&self, evolving_session: &[u64], k: usize, m: usize) -> BinaryHeap<SessionScore> {
        ...
        for (pos, item_id) in evolving_session.iter().rev().enumerate() {
            ...
            match hash_items.insert(*item_id, pos) {
                ...
                None => {
                    ...
                    match self.item_to_top_sessions_ordered.get(item_id) {
                        Some(similar_sessions) => {
                            let decay_factor = (len_evolving_session - pos) as f64 / len_evolving_session as f64;
                            ...
                            'session_loop: for session_id in similar_sessions {
                                match session_similarities.get_mut(session_id) {
                                    ...
                                    None => {
                                        let session_time_stamp = self.historical_sessions_max_time_stamp[*session_id as usize];
                                        if session_similarities.len() < m {
                                            session_similarities.insert(*session_id, decay_factor);
                                            heap_timestamps.push(SessionTime::new(session_time_stamp, *session_id));
                                        } else {
                                            ...
                                            if session_time_stamp > bottom.time {
                                                ...
                                                session_similarities.insert(*session_id, decay_factor);
                                                *bottom = SessionTime::new(session_time_stamp, *session_id);
                                            } else {
                                                break 'session_loop;
                                            }
                                        }
        ...
        for (session_id, score) in session_similarities.iter(){
            if closest_neighbors.len() < k {
                ...
                else if (score == &top.score) && (self.historical_sessions_max_time_stamp[*session_id as usize] > self.historical_sessions_max_time_stamp[top.id as usize]) {
                    let scored_session = SessionScore::new(*session_id, *score);
                    *top = scored_session;
                }
            }
