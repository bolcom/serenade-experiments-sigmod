use crate::vsknn::index::{VSKnnIndex, SessionTime};
...
impl TreeIndex {
    ...
    pub fn new(path_train: &str, n_most_recent_sessions: usize) -> Self {
        ...
        let (historical_sessions_train,
            ...
            historical_sessions_max_time_stamp) = data_train.unwrap();

        //println!("Creating tree index...");
        let (historical_sessions_item_id_sorted, historical_sessions_session_id_sorted) =
            prepare_binary_search(&historical_sessions_train, &historical_sessions_max_time_stamp, n_most_recent_sessions);
...
fn read_from_file(path: &str) -> Result<(Vec<Vec<usize>>, Vec<Vec<usize>>, Vec<u32>), Box<dyn Error>> {
...
fn prepare_binary_search(historical_sessions: &[Vec<usize>], timestamps: &[u32], n_most_recent_sessions: usize) -> (Vec<u64>, Vec<u32>) {
    ...
    let mut historical_sessions_values_sorted_truncated: Vec<u64> = Vec::with_capacity(historical_sessions_length);
    let mut historical_sessions_session_indices_sorted_truncated: Vec<u32> = Vec::with_capacity(historical_sessions_length);
    ...
    for current_item in unique_items.iter() {
        ...
        let mut current_item_similar_sessions: Vec<u64> = historical_sessions_values_sorted[left_index..right_index + 1].to_vec();
                    ...
                    if session_present {
                        ...
                        *value += decay_factor;
                        ...
                        if hash_similarities.len() < m {
                            ...
                            if session_time_stamp > heap_timestamps.peek().unwrap().time {
                                // Insert new key
                                hash_similarities.insert(*session_id, decay_factor);
