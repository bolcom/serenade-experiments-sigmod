...
pub fn read_from_file(path: &str) -> Result<(Vec<Vec<u64>>, Vec<Vec<usize>>, Vec<u32>), Box<dyn Error>> {
...
fn prepare_hashmap(historical_sessions: &[Vec<u64>], timestamps: &[u32], n_most_recent_sessions: usize) ->  HashMap<u64, Vec<u32>> {
...
impl VSKnnIndex for HashIndexNew {
    ...
    fn find_neighbors(&self, evolving_session: &[u64], k: usize, m: usize) -> BinaryHeap<SessionScore> {
        ...
        for (pos, item_id) in evolving_session.iter().enumerate() {
            ...
            match self.sessions_for_item.get(item_id) {
                        ...
                        match hash_similarities.get_mut(session_id) {
                            ...
                            None => {
                                ...
                                if hash_similarities.len() < m {
                                    ...
                                    heap_timestamps.push(
                                        SessionTime::new(session_time_stamp, *session_id));
                                } else {
