...
pub async fn v1_recommend(
    ...
    let k = data.neighborhood_size;
    let m = data.sample_size;
    ...
    let session_items = if user_consent {
        ...
        if session_items.is_empty() {
            ...
            if session_items.last().unwrap() != &most_recent_item {
                session_items.push(most_recent_item);
    ...
    let recommendations = vsknn::predict(vsknn_index, &session_items, k, m, how_many);
