...
use crate::vsknn::offline_index::OfflineIndex;
...
pub fn determine_training_data_statistics(descriptive_name: &str, training_data: &Vec<(u32, u64, u32)>) -> TrainingDataStats {
    let qty_records = training_data.len();

    let mut session_ids:Vec<u32> = training_data.into_par_iter().map(|(session_id, _item_id, _time) | session_id.clone()).collect();
    session_ids.par_sort_unstable();
    session_ids.dedup();
    let qty_unique_session_ids = session_ids.len();

    let mut item_ids:Vec<u64> = training_data.into_par_iter().map(|(_session_id, item_id, _time) | item_id.clone()).collect();
    item_ids.par_sort_unstable();
    item_ids.dedup();
    let qty_unique_item_ids = item_ids.len();

    let min_time = training_data.par_iter().map(| (_session_id, _item_id,
                                                      time) | time).min().unwrap();
    let max_time = training_data.par_iter().map(| (_session_id, _item_id,
                                                      time) | time).max().unwrap();
