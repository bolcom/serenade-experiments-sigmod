    ...
    if let Ok(lines) = read_lines(predictions_file) {
        ...
        for result in lines {
            if let Ok(line) = result {
                ...
                let recos = recos.split(",").collect_vec().into_iter().map(|x| x.clone().parse::<u64>().unwrap()).collect_vec();
                let next_items = *unsafe { recos_w_predictions.get_unchecked(1) };
                let next_items = next_items.split(",").collect_vec().into_iter().map(|x| x.clone().parse::<u64>().unwrap()).collect_vec();
                // recos.iter
