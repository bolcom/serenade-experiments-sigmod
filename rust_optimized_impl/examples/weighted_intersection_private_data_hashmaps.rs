...
fn main() {
    ...
    let (historical_sessions_train, historical_sessions_id_train, historical_sessions_max_time_stamp_train) = data_train.unwrap();
    ...
    let (historical_sessions_test, historical_sessions_id_test, historical_sessions_max_time_stamp_test) = data_test.unwrap();
    ...
    let algorithms = ["binary_search_heap+tree", "binary_search_heap+vecs", "binary_search_heap+hash", "hashmap+heap+hash      ", "hashmap+heap+hash_adj  "];

    // Prepare algorithms
    let (historical_sessions_item_id_sorted, historical_sessions_session_id_sorted) = prepare_binary_search(&historical_sessions_train);
    let (historical_sessions_item_id_sorted_truncated, historical_sessions_session_id_sorted_truncated) = prepare_binary_search_truncated(&historical_sessions_train, &historical_sessions_max_time_stamp_train, n_most_recent_sessions);
    let historical_sessions_hashed = prepare_hashmap(&historical_sessions_item_id_sorted, &historical_sessions_session_id_sorted);
    let historical_sessions_hashed_truncated = prepare_hashmap(&historical_sessions_item_id_sorted_truncated, &historical_sessions_session_id_sorted_truncated);
    ...
    for i in 0..n_iterations {
        ...
        let closest_neighbors_binary_search_heapplustree = calc_similarities_binary_search_heapplustree(&evolving_session, &historical_sessions_item_id_sorted_truncated, &historical_sessions_session_id_sorted_truncated, &historical_sessions_max_time_stamp_train, n_most_recent_sessions, k);
        duration[i] += start.elapsed();
        // Binary search + heap + vectors
        let start = Instant::now();
        let closest_neighbors_binary_search_heapplusvec = calc_similarities_binary_search_heapplusvectors(&evolving_session, &historical_sessions_item_id_sorted_truncated, &historical_sessions_session_id_sorted_truncated, &historical_sessions_max_time_stamp_train, n_most_recent_sessions, k);
        duration[i + n_iterations] += start.elapsed();
        // Binary search + heap + hashmap
        let start = Instant::now();
        let closest_neighbors_binary_search_heapplushash = calc_similarities_binary_search_heapplushashmap(&evolving_session, &historical_sessions_item_id_sorted_truncated, &historical_sessions_session_id_sorted_truncated, &historical_sessions_max_time_stamp_train, n_most_recent_sessions, k);
        duration[i + 2 * n_iterations] += start.elapsed();
        // Hashmap + heap + hashmap
        let start = Instant::now();
        let closest_neighbors_hashheapplushash = calc_similarities_hashplusheapplushash(&evolving_session, &historical_sessions_hashed_truncated, &historical_sessions_max_time_stamp_train, n_most_recent_sessions, k);
        duration[i + 3 * n_iterations] += start.elapsed();
        // Hashmap + heap + hashmap
        let start = Instant::now();
        let closest_neighbors_hashheapplushashadj = calc_similarities_hashplusheapplushashadj(&evolving_session, &historical_sessions_hashed_truncated, &historical_sessions_max_time_stamp_train, n_most_recent_sessions, k);
    ...
    for (i, algorithm) in algorithms.iter().enumerate() {
        print_timing(&mut duration[(i * n_iterations)..(i + 1) * n_iterations], &n_iterations, algorithm);
    }
}
...
fn print_timing(duration: &mut [Duration], n_iterations: &usize, name: &str) {
    ...
    let ns_avg = ((duration_total.as_secs() * 1_000_000_000 + (duration_total.subsec_nanos() as u64)) / *n_iterations as u64).to_formatted_string(&Locale::en);
    let ns_max = (duration_max.as_secs()  * 1_000_000_000 + (duration_max.subsec_nanos() as u64)).to_formatted_string(&Locale::en);
    let ns_p90 = ((duration_p90.as_secs() * 1_000_000_000 + (duration_p90.subsec_nanos() as u64)) / n_iterations_p90 as u64).to_formatted_string(&Locale::en);
    println!("test {:}: {:>20} ns/iter (average) | {:>15} ns/iter (p90) | {:>15} ns/iter (max)", name, ns_avg, ns_p90, ns_max);
}
...
fn read_from_file(path: &str, max_events: usize) -> Result<(Vec<Vec<usize>>, Vec<Vec<usize>>, Vec<usize>), Box<dyn Error>> {
    ...
    let session_id_sorted: Vec<usize> = session_id_indices.iter().map(|&i| session_id[i].clone()).collect();
    let item_id_sorted: Vec<usize> = session_id_indices.iter().map(|&i| item_id[i].clone()).collect();
    let time_sorted: Vec<usize> = session_id_indices.iter().map(|&i| time[i].clone()).collect();
    ...
    let mut historical_sessions_max_time_stamp: Vec<usize> = Vec::with_capacity(session_id_unique.len());
    ...
    Ok((historical_sessions, historical_sessions_id, historical_sessions_max_time_stamp))
}

fn prepare_hashmap(historical_sessions_item_id_sorted: &[usize], historical_sessions_session_id_sorted: &[usize]) -> HashMap<usize, Vec<usize>> {
    ...
    for (key, current_item) in historical_sessions_item_ids.iter().enumerate() {
        let left_index = binary_search_left(historical_sessions_item_id_sorted, current_item).unwrap();
        let right_index = binary_search_right(historical_sessions_item_id_sorted, current_item).unwrap();
        let sessions: Vec<usize> = historical_sessions_session_id_sorted[left_index..right_index + 1].to_vec();
...
fn prepare_binary_search(historical_sessions: &[Vec<usize>]) -> (Vec<usize>, Vec<usize>) {
    ...
    let historical_sessions_values_sorted: Vec<usize> = historical_sessions_indices.iter().map(|&i| historical_sessions_values[i].clone()).collect();
    let historical_sessions_session_indices_sorted: Vec<usize> = historical_sessions_indices.iter().map(|&i| historical_sessions_session_indices[i].clone()).collect();
...
fn prepare_binary_search_truncated(historical_sessions: &[Vec<usize>], timestamps: &[usize], n_most_recent_sessions: usize) -> (Vec<usize>, Vec<usize>) {
    ...
    let historical_sessions_values_sorted: Vec<usize> = historical_sessions_indices.iter().map(|&i| historical_sessions_values[i].clone()).collect();
    let historical_sessions_session_indices_sorted: Vec<usize> = historical_sessions_indices.iter().map(|&i| historical_sessions_session_indices[i].clone()).collect();
    let historical_sessions_timestamps_sorted: Vec<usize> = historical_sessions_indices.iter().map(|&i| historical_sessions_timestamps[i].clone()).collect();

    // Get unique item_ids
    let mut unique_items = historical_sessions_values_sorted.clone();
    unique_items.dedup();
    
    let mut historical_sessions_values_sorted_truncated: Vec<usize> = Vec::with_capacity(historical_sessions_length);
    let mut historical_sessions_session_indices_sorted_truncated: Vec<usize> = Vec::with_capacity(historical_sessions_length);
    ...
    for current_item in unique_items.iter() {
        let left_index = binary_search_left(&historical_sessions_values_sorted, current_item).unwrap();
        let right_index = binary_search_right(&historical_sessions_values_sorted, current_item).unwrap();
        let mut current_item_similar_sessions: Vec<usize> = historical_sessions_values_sorted[left_index..right_index + 1].to_vec();
        let mut current_item_timestamps: Vec<usize> = historical_sessions_timestamps_sorted[left_index..right_index + 1].to_vec();
        let mut current_item_similar_sessions_ids: Vec<usize> = historical_sessions_session_indices_sorted[left_index..right_index + 1].to_vec();
        ...
        let mut current_item_similar_sessions_id_sorted: Vec<usize> = timestamp_indices.iter().map(|&i| current_item_similar_sessions_ids[i].clone()).collect();
        ...
        historical_sessions_session_indices_sorted_truncated.append(&mut current_item_similar_sessions_id_sorted);
    }

    
    // Return sorted flattened historical sessions and sorted flattened historical session indices
    (historical_sessions_values_sorted_truncated, historical_sessions_session_indices_sorted_truncated)
    // (historical_sessions_values_sorted, historical_sessions_session_indices_sorted)
...
fn insert_and_remove<T>(mut vector: Vec<T>, index_insert: usize, index_remove: usize, elem_insert: T) -> Vec<T> {
    ...
    unsafe {
        if index_insert < index_remove {
            ptr::copy(vec_ptr.offset(index_insert as isize),
            ...
            ptr::copy(vec_ptr.offset(index_remove as isize + 1),
...
fn calc_similarities_binary_search_heapplustree(
    ...
    for (pos, item_id) in evolving_session.iter().enumerate() {
        ...
        if right_index_result.is_ok() {
            let left_index = binary_search_left(historical_sessions_item_id_sorted, item_id).unwrap();
            ...
            let similar_sessions = &historical_sessions_session_id_sorted[left_index..right_index + 1];
...
fn calc_similarities_binary_search_heapplusvectors(
    ...
    let mut heap_timestamps = BinaryHeap::<SessionTime>::with_capacity(n_most_recent_sessions);
    let mut session_ids_sorted_by_session: Vec<usize> = Vec::with_capacity(n_most_recent_sessions + 1);
    let mut similarity_score_sorted_by_session: Vec<f32> = Vec::with_capacity(n_most_recent_sessions + 1);
                ...
                if session_index_result.is_ok() {
                    ...
                    // Once we reach n_most_recent_sessions in the output, only store new sessions if they are newer than the existing minimum timestamp.
                        ...
                        if session_time_stamp > heap_timestamps.peek().unwrap().0 {
                            ...
                            // Remove the the existing minimum time stamp.
                            let session_id_remove = heap_timestamps.peek().unwrap().1;
                            let session_index_remove = session_ids_sorted_by_session.binary_search(&session_id_remove).unwrap();
                            // Fused insert-and-remove for the vectors reduces computation time
                            session_ids_sorted_by_session = insert_and_remove(session_ids_sorted_by_session, session_index, session_index_remove, *session_id);
                            similarity_score_sorted_by_session = insert_and_remove(similarity_score_sorted_by_session, session_index, session_index_remove, decay_factor);
                            // Set new minimum timestamp in heap_timestamp
                            let mut bottom  = heap_timestamps.peek_mut().unwrap();
                            *bottom = SessionTime(session_time_stamp, *session_id);
                        }
...
fn calc_similarities_binary_search_heapplushashmap(
    ...
    let mut heap_timestamps = BinaryHeap::<SessionTime>::with_capacity(n_most_recent_sessions);
    let mut hash_similarities: HashMap<usize, f32> = HashMap::with_capacity(n_most_recent_sessions + 1);
