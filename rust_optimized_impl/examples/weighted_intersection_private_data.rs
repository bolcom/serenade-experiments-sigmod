...
fn main() {
    ...
    for i in 0..n_iterations {
        ...
        let (similarities_binary_search_index, similarities_binary_search_values) = calc_similarities_binary_search(&evolving_session, &historical_sessions_item_id_sorted, &historical_sessions_session_id_sorted, &historical_sessions_max_time_stamp_train, n_most_recent_sessions);
...
fn calc_similarities_nested_loops(evolving_session: &[usize], historical_sessions: &[HashSet<usize>]) -> Vec<f32> {
...
fn calc_similarities_binary_search(
    ...
    for (pos, item_id) in evolving_session.iter().enumerate() {
        ...
        if right_index_result.is_ok() {
            ...
            for session_id in historical_sessions_session_id_sorted[left_index..right_index + 1].iter(){
...
fn calc_similarities_linear_search(
    ...
    let mut session_ids_sorted_by_timestamp: Vec<usize> = Vec::with_capacity(n_most_recent_sessions + 1);
    let mut similarity_score_sorted_by_session: Vec<f32> = Vec::with_capacity(n_most_recent_sessions + 1);
    let mut timestamps_sorted_by_timestamp: Vec<usize> = Vec::with_capacity(n_most_recent_sessions + 1);
    ...
    for (session_id, session) in historical_sessions.iter().enumerate(){
        ...
        if session_ids_sorted_by_session.len() < n_most_recent_sessions {
            ...
            if similarity_score > 0_f32 {
                let session_time_stamp_index = binary_search_reverse(&timestamps_sorted_by_timestamp, &session_time_stamp).unwrap_or_else(|x| x);
                // Insert
    ...
    // Return session_id and corresponding similarity score to evolving session for up to n_most_recent_sessions
    (session_ids_sorted_by_session, similarity_score_sorted_by_session)
}