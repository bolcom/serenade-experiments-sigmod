...
fn main() {
    ...
    // Prepare algorithms
    let (historical_sessions_item_id_sorted, historical_sessions_session_id_sorted) = prepare_binary_search(&historical_sessions);
    let (historical_sessions_item_id_sorted_eytzinger, eytzinger_permutation) = prepare_eytzinger_search(&historical_sessions_item_id_sorted);
    ...
    let mut rng = rand::thread_rng();
    let algorithms = ["binary_search", "eytzgr_search", "nested_loops ", "linear_search"];
    ...
    for i in 0..n_iterations {
        ...
        let similarities_binary_search = calc_similarities_binary_search(&evolving_session, &historical_sessions_item_id_sorted, &historical_sessions_session_id_sorted, NUM_HISTORY_SESSIONS);
        duration[i] += start.elapsed();
        // Eytzinger search
        let start = Instant::now();
        let similarities_eytzinger_search = calc_similarities_eytzinger_search(&evolving_session, &historical_sessions_item_id_sorted, &historical_sessions_item_id_sorted_eytzinger, &eytzinger_permutation, &historical_sessions_session_id_sorted, NUM_HISTORY_SESSIONS);
        ...
        // Linear search
        let start = Instant::now();
        let similarities_linear_search = calc_similarities_linear_search(&evolving_session, &historical_sessions_item_id_sorted, &historical_sessions_session_id_sorted, NUM_HISTORY_SESSIONS);
        duration[i + 3 * n_iterations] += start.elapsed();
        assert_eq!(similarities_binary_search, similarities_linear_search);
    }
    for (i, algorithm) in algorithms.iter().enumerate() {
        print_timing(&duration[(i * n_iterations)..(i + 1) * n_iterations], &n_iterations, algorithm);
    }
}

fn print_timing(duration: &[Duration], n_iterations: &usize, name: &str) {
    let duration_total: Duration = duration.iter().sum();
    let ns_per_iter = (duration_total.as_secs() * 1_000_000_000 + (duration_total.subsec_nanos() as u64)) / *n_iterations as u64;
...
fn get_evolving_session(rng: &mut ThreadRng) -> Vec<usize> {
    let evolving_session: Vec<usize>  = (0..NUM_ITEMS_IN_EVOLVING_SESSION).map(|_| rng.gen_range(0..MAX_ITEM_ID)).collect();
...
fn calc_similarities_binary_search(
    ...
    for (pos, item_id) in evolving_session.iter().enumerate() {
        ...
        if right_index_result.is_ok() {
            ...
            let decay_factor = (pos + 1) as f32 / evolving_session.len() as f32;
            // Fill similarities
            let current_similarity_index = &historical_sessions_session_id_sorted[left_index..right_index + 1];
            current_similarity_index.iter().for_each(|i| similarities[*i] += decay_factor);
        }
    }
    similarities
}
...
fn eytzinger(array: &Vec<usize>, array_e: &mut Vec<usize>, permutation: &mut Vec<usize>, mut ind_arr: usize, mut ind_eyt: usize, n: usize) -> (usize) {
    if (ind_eyt <= n){
        ...
        if index.is_ok() {
            ...
            while next_index < first_index {
                if historical_sessions_item_id_sorted[next_index] == *item_id {
                    let current_similarity_index = historical_sessions_session_id_sorted[next_index];
                    similarities[current_similarity_index] += decay_factor;
                    next_index -= 1;
                } else {
