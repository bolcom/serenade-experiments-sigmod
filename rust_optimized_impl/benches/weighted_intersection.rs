...
fn bench_sprs_matrix_vector_multiplication(bench: &mut Bencher) {
    ...
    bench.iter(|| {
        bencher::black_box(sprs_matrix_vector_multiplication(&evolving_session, &historical_sessions));
    });
}

fn sprs_matrix_vector_multiplication(
    evolving_session: &CsMat<f64>,
    historical_session: &CsMat<f64>) -> CsMat<f64> {
...
fn bench_torch_matrix_vector_multiplication(bench: &mut Bencher) {
    ...
    let weights = tch::Tensor::arange(NUM_ITEMS_IN_EVOLVING_SESSION as f64, (Kind::Float, cpu)).unsqueeze(-1);
...
fn torch_matrix_vector_multiplication(
    ...
    (weights * historical_sessions.index_select(0_i64, evolving_session)).sum1(&[0_i64], false, Kind::Float)
}
...
fn bench_intersection(bench: &mut Bencher) {
    ...
    bench.iter(|| {
        bencher::black_box(intersection(&sorted_evolving_session, &position_weights, &historical_sessions));
    });
}
...
fn intersection(
    ...
    for neighbor_session in sorted_history_sessions.iter() {
        ...
        while i < m && j < n {
            let i_smaller_j = unsafe { sorted_evolving_session.get_unchecked(i) } < unsafe { neighbor_session.get_unchecked(j) };
            let j_smaller_i = unsafe { neighbor_session.get_unchecked(j) } < unsafe { sorted_evolving_session.get_unchecked(i) };
            let i_equal_j = !i_smaller_j && !j_smaller_i;

            similarity = similarity + (i_equal_j as usize) as f64 * unsafe { position_weights.get_unchecked(i) };
...
fn bench_binarysearch(bench: &mut Bencher) {
    ...
    let evolving_session: Vec::<i64>  = (0..items_session).map(|_| rng.gen_range(0..items)).collect::<Vec<i64>>();
    ...
    let mut historical_sessions_indices = (0..historical_sessions_length as i64).collect::<Vec<i64>>();
    historical_sessions_indices.sort_by_key(|&i| &historical_sessions_values[i as usize]);
    let historical_sessions_values_sorted = historical_sessions_indices.iter().map(|&i| historical_sessions_values[i as usize].clone()).collect();
    let historical_sessions_session_indices_sorted = historical_sessions_indices.iter().map(|&i| historical_sessions_session_indices[i as usize].clone()).collect();
...
fn binarysearch(
    ...
    for (pos, item_id) in evolving_session.iter().enumerate() {
        ...
        if index.is_ok() {
            ...
            let current_similarity_index = historical_sessions_session_indices_sorted[first_index] as usize;
            ...
            while left_index >= 0 {
                if (historical_sessions_values_sorted[left_index as usize] == *item_id) {
                    let current_similarity_index = historical_sessions_session_indices_sorted[left_index as usize] as usize;
                    similarities[current_similarity_index] += decay_factor;
                }
                left_index -= 1;
            }
            ...
            while right_index < historical_sessions_values_sorted.len() {
                if (historical_sessions_values_sorted[right_index] == *item_id) {
                    let current_similarity_index = historical_sessions_session_indices_sorted[right_index] as usize;
                    similarities[current_similarity_index] += decay_factor;
                }
                right_index += 1;
            }
