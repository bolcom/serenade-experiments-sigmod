...
benchmark_group!(benches, bench_nested_loops, bench_binary_search, bench_linear_search);
...
fn bench_binary_search(bench: &mut Bencher) {
    ...
    // Prepare algorithms
    let (historical_sessions_values_sorted, historical_sessions_session_indices_sorted) = prepare_binary_search(&historical_sessions);

    // Run bencher
    bench.iter(|| {
        bencher::black_box(binary_search(&evolving_session, &historical_sessions_values_sorted, &historical_sessions_session_indices_sorted));
    });    
}
...
fn bench_linear_search(bench: &mut Bencher) {
    ...
    // Run bencher
    bench.iter(|| {
        bencher::black_box(linear_search(&evolving_session, &historical_sessions_values_sorted, &historical_sessions_session_indices_sorted));
    });    
}

fn get_evolving_session(rng: &mut ThreadRng) -> Vec<u64> {
    let evolving_session: Vec<u64>  = (0..NUM_ITEMS_IN_EVOLVING_SESSION).map(|_| rng.gen_range(0..MAX_ITEM_ID)).collect();
...
fn prepare_binary_search(historical_sessions: &Vec<Vec<u64>>) -> (Vec<u64>, Vec<u64>) {
    ...
    let historical_sessions_values_sorted: Vec<u64> = historical_sessions_indices.iter().map(|&i| historical_sessions_values[i as usize].clone()).collect();
    let historical_sessions_session_indices_sorted: Vec<u64> = historical_sessions_indices.iter().map(|&i| historical_sessions_session_indices[i as usize].clone()).collect();
    ...
    for (pos, item_evolving_session) in evolving_session.iter().enumerate() {
        ...
        for (item_id, item) in historical_sessions_values_sorted.iter().enumerate(){
                if *item_evolving_session == *item {
                    let current_similarity_index = historical_sessions_session_indices_sorted[item_id] as usize;
                    similarities[current_similarity_index] += decay_factor;
                }
            }
    }
