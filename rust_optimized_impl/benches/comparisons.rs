...
use hashbrown::{HashSet as HashSetBrown, HashSet, HashMap};

use rayon::prelude::*;

use serenade_optimized::io::{read_training_data, read_test_data_evolving};
use serenade_optimized::vsknn::index::SessionTime;
use serenade_optimized::io::{TrainingSessionId, ItemId, Time};
...
fn zipper(bench: &mut Bencher) {
    ...
    let test_data: HashMap<TrainingSessionId, Vec<ItemId>> = read_test_data_evolving(TEST_DATA_FILE);
...
fn sanitize_training_data(training_data: Vec<(TrainingSessionId, ItemId, Time)>, sample_size_m: usize, max_qty_session_items: usize) -> Vec<(TrainingSessionId, ItemId, Time)>{
    ...
    let filtered_training_data: Vec<(TrainingSessionId, ItemId, Time)> = training_data.par_iter()
    ...
    let valid_session_ids: HashSet<u32> = filtered_training_data
        .iter()
        .map(|(session_id, item_id, time)| (item_id, SessionTime::new(session_id.clone(), time.clone())))
        .into_group_map()
        .into_iter()
...
// we remove the sessions per item with the lowest time values
            ...
            let session_ids: HashSet<u32> = session_id_with_time.iter()
                .map(|session_id_time| session_id_time.session_id).collect();
            session_ids
        }).collect();

    training_data.into_iter().filter(|training_event| valid_session_ids.contains(&training_event.0)).cloned().collect()
}