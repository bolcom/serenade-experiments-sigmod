...
if [[ $# -ne 4 ]]; then
    ...
    echo "Usage: " $0 ../datasets/private-clicks-1m_train.txt ../datasets/private-clicks-1m_test.txt duckdb_predictions.txt duckdb_latencies.csv
    exit 2
fi
training=$1
test=$2
...
function do_run() {
  ...
  pred_out=$3
  latency_out=$4
  python3 main_duckdb.py ${train_in} ${test_in} ${pred_out} ${latency_out}
}
