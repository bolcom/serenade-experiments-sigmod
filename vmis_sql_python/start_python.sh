...
if [[ $# -ne 4 ]]; then
    ...
    echo "Usage: " $0 ../datasets/bolcom-clicks-1m_train.txt ../datasets/bolcom-clicks-1m_test.txt vsknn_predictions.txt prediction_latencies.csv
    exit 2
fi
training=$1
test=$2
...
function do_run() {
  if [[ $# -ne 4 ]]; then
    ...
    echo "Usage: " $0 ../datasets/bolcom-clicks-1m_train.txt ../datasets/bolcom-clicks-1m_test.txt vsknn_predictions.txt position_latency.txt
    exit 2
  fi
  train_in=$1
  test_in=$2
